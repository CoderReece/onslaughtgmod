...
ENT.PrintName = "Ally Rebel"
ENT.Category = "SNPCs"
ENT.Walk = ACT_RUN_AIM_RIFLE
ENT.Idle = ACT_IDLE_ANGRY_SMG1
ENT.Model = "models/humans/group03/male_01_bloody.mdl"
...
function ENT:GetGuns()
	return {"combine_ar2","combine_shotgun","combine_smg1","combine_pistol"}
end
...
function ENT:GetAimVector()
	return self.Enemy and (self.Enemy:GetPos()+(Vector(0,0,40)*self.Enemy:GetModelScale())-self:GetShootPos()):GetNormal() or self:GetForward()
end
...
function ENT:FireWeapon()
	if not self:CanFireGun() then return 0.1 end
	local wep = self.Weapon
	if wep.NPCShoot_Primary then
...
function ENT:MoveToPos( pos, options )
	...
	while ( path:IsValid() ) and IsValid(self) do
		if self:EnemyInRange() and not self.DontShootScoot then
			self:StartActivity(self:GetWeaponAttackAnim())
			self:AimAndFire()
...
function ENT:RunBehaviour()
	while ( true ) do
		if not self.Idling and (self.NextCheckEnemy or 0) < CurTime() then
		...
		if (IsValid(self.Enemy) and self.Enemy:Alive()) then
			...
			if self:EnemyInRange() then
				...
				self:MoveToPos(self.Enemy:GetPos())
			end
		else
			self:StartActivity(self.Idle)
		end			
